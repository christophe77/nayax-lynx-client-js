...
- `LICENSE` (MIT) and `CHANGELOG.md` included in the published npm tarball.
- `prepublishOnly` script (typecheck, test, build) before `npm publish`.

### Changed

- README: clarify that Lynx has no HTTP webhooks (operational events via Amazon SQS);
  reframe `nayax-lynx-client-js/webhooks` as generic HMAC helpers, not Lynx-specific.
- Default `User-Agent` bumped to `nayax-lynx-client-js/0.1.1`.
