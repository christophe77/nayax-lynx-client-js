...
- **Typed errors**: `NayaxError`, `NayaxAuthError`, `NayaxRateLimitError`, `NayaxNetworkError`, `NayaxTimeoutError`.
...
> devzone. The library also exposes legacy *inferred* helpers
> (`client.getTransactionsByMachine`, `client.getMachineTelemetry`, `client.updateInventory`)
> and *inferred* placeholder resources (`client.transactions`, `client.telemetry`,
> `client.inventory`) that pre-date the doc-driven implementation. Prefer the documented
> equivalents:
...
>
> **No Lynx webhooks.** Nayax Lynx is a request/response REST API — it does **not** expose
> webhooks. Operational events (transactions, EVA-DTS, etc.) are delivered via
> **Amazon SQS**; see Nayax's [SQS setup guide](https://devzone.nayax.com/docs/manage-data-operations/amazon-sqs/overview).
> The `nayax-lynx-client-js/webhooks` sub-export ships a *generic* HMAC verifier
> (originally built for the project brief) that you can use with any other provider — it is
> not tied to a Lynx-side feature.
...
## Generic HMAC webhook helpers (not Lynx)

> **This section does not apply to Lynx itself.** Lynx delivers events through Amazon SQS,
> not HTTP webhooks. The helpers below are a generic HMAC signature verifier that ships in
> this package because the original project brief asked for one — feel free to use them with
> any *other* webhook provider you have (Stripe, a Spark integration, your own emitter…).
...
Signature verification is timing-safe (`crypto.timingSafeEqual`).
